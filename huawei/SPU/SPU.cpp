...
void SPU_execute_O_OUT(SPU* processor, SPUInstruction instr)
{
    ...
    //printf_yellow("Executing %cOUT 0x%x 0x%x %d\n", (instr.opcode & ARG_TYPE_OPCODE_MASK) == ARG_TYPE_PTR ? '$' : ' ', port_number, abs_data_ptr, count);
...
void SPU_execute_O_IN(SPU* processor, SPUInstruction instr)
{
    ...
    //printf_yellow("Executing OUT 0x%x 0x%x\n", port_number, relative_data_ptr);
    ...
    if (port_number >= port_out_handlers_number || port_out_handlers[port_number] == NULL)
    {
        return; // UNKNOWN PORT
    }
...
{
    int32_t args[4] = {};
    memcpy(args, instr.args_ptr, 4 * sizeof(args[0]));
    ...
    //printf_yellow("CMOV 0x%x 0x%x %d -- ", abs_dst_ptr, abs_src_ptr, count);
...
void SPU_handle_out_VGA(SPU* processor, uint32_t data_ptr, uint32_t count)
{
    ...
    if (VGA_window == NULL)
    {
        ...
        VGA_window = SDL_CreateWindow("SPU VGA", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, VGA_screen_width, VGA_screen_height, SDL_WINDOW_SHOWN);
    ...
    for (int y = 0; y < VGA_screen_height; ++y)
    {
        ...
        {
            offset = (uint32_t)y * sizeof(uint32_t) * VGA_screen_width + (uint32_t)x * sizeof(uint32_t);
            a = SPU_read_memory_cell(processor, data_ptr + offset);
            memcpy(pixels_arr + y * window_surface->pitch + (uint32_t)x * sizeof(uint32_t), &a, sizeof(uint32_t));
        }*/
        offset = (uint32_t)y * sizeof(uint32_t) * VGA_screen_width;
        if (offset < count)
            SPU_read_memory(processor, pixels_arr + y * window_surface->pitch, data_ptr + offset, VGA_screen_width * sizeof(uint32_t));
    }
